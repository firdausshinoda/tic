...
@elseif($page=="journal-edit-video")
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form id="formExecuteModal" class="formExecuteModal">
                <div class="modal-header">
                    <h6 class="modal-title text-default font-weight-bold" id="exampleModalLabel">VIDEO</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <label for="link_video_youtube" class="form-label mb-0"><b><code>*)</code>Link Youtube</b></label><br/>
                        <textarea class="form-control rounded" id="link_video" name="link_video" placeholder="Link youtube example : https://www.youtube.com/watch?v=xxxxxxxx" rows="2">{{$data->link_video}}</textarea>
                        <input type="hidden" id="no_abs" name="no_abs" value="{{$data->no_abstrak}}">
                    </div>
                    <hr/>
    ...
    <script type="text/javascript">
        $(function() {
            $('#formExecuteModal').validate({
                ...
                messages: {
                    link_video: { required: "Please fill in", },
                },
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        preConfirm: function () {
                            return new Promise(function (resolve) {
                                ...
                                $.ajax({
                                    type: "POST",data:values, url: "{{ url('/api/updateMyJournalVideo') }}",
...
@elseif($page=="upload-journal")
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title text-default font-weight-bold" id="exampleModalLabel">UPLOAD FULL PAPER</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <small><code>Max 3 Mb</code></small>
                <form action="{{url('api/updateMyJournalPaperUpload')}}" id="drop-zone" class="dz dropzone">
                    @csrf
                    <input type="hidden" name="kode" value="{{$no_abs}}">
                </form>
    ...
    <script type="text/javascript">
        new Dropzone("#drop-zone", {
            ...
            maxFilesize: 3, // MB
            maxFiles: 1,
            dictDefaultMessage: "Drop File Here or Click To Search Doc/Docx File Full Paper...",
            init: function() {
                var known = false;
                ...
                this.on("success", function(file, responseText) {
                    if (responseText.status === "OK"){
                        ...
                        swall_failed_text(responseText.message+"AAA");
                    }
                });
                this.on('error', function(file) {
                    ...
                    if (file.size > 3000000){
                        swall_failed_text('Max file size 3 Mb')
                    } else if (file_type !== "doc" || file_type !== "docx"){
                        swall_failed_text('Only type file DOC or DOCXXX')
                    } else {
                        swall_failed_text('Failed to upload file...')
                    }
                });
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <form id="formExecuteModal" class="formExecuteModal" enctype="multipart/form-data">
                <div class="modal-header">
                    <h6 class="modal-title text-default font-weight-bold" id="exampleModalLabel">EDIT PHOTO AUTHOR</h6>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <div class="modal-body">
                    <div class="form-group mb-0">
                        ...
                        <div class="mb-3 text-center">
                            <img src="<?= empty(Session::get('foto_author')) ? asset('img/user_default.jpg') : asset('/upload/author/'.Session::get('foto_author')); ?>" class="center-cropped w-50" id="PreviewImg">
                        </div>
                        <div class="custom-file">
                            ...
                            <input type="hidden" id="foto_lama" name="foto_lama" value="{{Session::get('foto_author')}}">
                        </div>
                    </div>
                    <hr/>
                    <div class="text-right">
    ...
    <script type="text/javascript">
        ...
        $(function() {
            $('#formExecuteModal').validate({
                ...
                messages: {
                    foto: {required: "Please fill in", extension: "Only PNG , JPEG , JPG...", filesize: "Maksimal size 1 Mb", },
                },
                errorElement : 'div', errorPlacement: function(error, element) {
                    var placement = $(element).data('error');
                    if (placement) {$(placement).append(error)} else {error.insertAfter(element);}
                        ...
                        preConfirm: function () {
                            return new Promise(function (resolve) {
                                ...
                                $image_crop.croppie('result', {
                                    ...
                                    $.ajax({
                                        type: "POST",data:values,url: "{{ url('/api/updateAccountAuthorPhoto') }}",
                    ...
                    swalWithBootstrapButtons.fire({
                        ...
                        preConfirm: function () {
                            return new Promise(function (resolve) {
                                var values = $('#formExecuteModal').serialize();
                                $.ajax({
                                    type: "POST",data:values, url: "{{ url('/api/updateAccountAuthorPassword') }}",
                                    headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')}, dataType: 'json',
                                    success: function(response) {
                                        loader_hide();
                                        if (response.status === "OK"){
...
@elseif($page=="revision-author")
    <div class="modal-dialog"role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h6 class="modal-title text-default font-weight-bold" id="exampleModalLabel">REVISION JOURNAL</h6>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            ...
            <div class="modal-body">
                <small><code>Max 5 MB</code></small>
                <form action="{{url('api/updateRevisionAuthor')}}" id="drop-zone" class="dz dropzone text-center">
                    @csrf
                    <input type="hidden" name="no_abs" value="{{$no_abs}}">
    ...
    <script type="text/javascript">
        new Dropzone("#drop-zone", {
            ...
            maxFilesize: 5, // MB
            maxFiles: 1,
            dictDefaultMessage: "Drop File Here or Click To Search Doc/Docx File Full Paper Revision...",
            init: function() {
                var known = false;
                var submitButton = document.querySelector("#submit-all");
                var myDropzone = this;
                ...
                this.on('error', function(file) {
                    ...
                    if (file.size > 5000000){
                        swall_failed_text('Max file size 5 Mb')
                    } else if (file_type !== "doc" || file_type !== "docx"){
                        swall_failed_text('Only type file DOC or DOCX')
                    } else {
                        swall_failed_text('Failed to upload file...')
                    }
                });
