...
@section('konten')
    ...
    <a href="{{url('/admin/degree/add')}}" class="btn btn-default btn-lg btn-floating shadow rounded-circle">
            ...
            $('#dt_table').DataTable({
                ...
                columns: [
                    ...
                    { "render": function ( data, type, row )
                        {
                            return '<a href="{{url('/admin/degree/edit?id=')}}'+row.id_gelar+'" class="btn btn-sm btn-info m-1 btn-block">EDIT</a>' +
                                '<button type="button" class="btn btn-sm btn-danger m-1 btn-block" onclick="ajaxDelete(\'degree\',\''+row.id_gelar+'\')"><i class="far fa-trash-alt"></i></button>'
                        }
                    },
                ]
            });
