...
@section('konten')
    ...
    <a href="{{url('/admin/sosmed/add')}}" class="btn btn-default btn-lg btn-floating shadow rounded-circle">
        ...
        $(document).ready(function() {
            ...
            $('#dt_table').DataTable({
                ...
                columns: [
                    ...
                    { "render": function ( data, type, row )
                        {
                            return '<a href="{{url('/admin/sosmed/edit?id=')}}'+row.id_sosmed+'" class="btn btn-sm btn-info m-1 btn-block">EDIT</a>' +
                                '<button type="button" class="btn btn-sm btn-danger m-1 btn-block" onclick="ajaxDelete(\'sosmed\',\''+row.id_sosmed+'\')"><i class="far fa-trash-alt"></i></button>'
                        }
                    },
                ]
            });
