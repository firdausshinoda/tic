...
@section('js')
    <script type="text/javascript">
            ...
            $('#dt_table').DataTable({
                ...
                columns: [
                    ...
                    { "render": function ( data, type, row )
                        {
                            return '<a href="{{url('admin/revision/detail?abs=')}}'+row.no_abstrak+'">'+row.no_abstrak+'</a>';
                    ...
                    { "render": function ( data, type, row )
                        {
                            if (row.stt_pembayaran_konfirmasi  === "ACCEPTED" && row.stt_jurnal === "ACCEPTED" && row.file_nama !== null){
                                return '<button type="button" class="btn btn-sm btn-success m-1 btn-block" onclick="acc_publish(\''+row.no_abstrak+'\')">COPYEDITING</button>'
                            } else {
                                return '';
                            }
                        }, "className": "dt-center"
        ...
        function acc_publish(no_abs) {
            swalWithBootstrapButtons.fire({
                text: 'Are you sure you want to confirm accepted journal to copyediting?',
                icon: 'warning',
                ...
                preConfirm: function () {
                    return new Promise(function (resolve) {
                        ...
                        $.ajax({
                            type: "POST",data:values, url: "{{ url('/api/updateRevisionAcc') }}",
