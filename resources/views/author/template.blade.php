...
<head>
    ...
    <link href="{{ asset('/custom/custom_user.css?version=1')}}" rel="stylesheet">
...
<header>
    <nav class="navbar navbar-expand-md navbar-light bg-white fixed-top pl-sm-4 pr-sm-5" style="box-shadow: 0 5px 5px -5px #999,0 -1px 0 0 #fff;">
        <div class="navbar-brand pl-md-5">
            ...
            <a class="text-dark text-decoration-none" href="{{ url('/author') }}">
                <b>AUTHOR</b>
            </a>
        ...
        <div class="collapse navbar-collapse pr-5" id="navbarCollapse">
            <form class="form-inline form-inline-custom ml-5 display-none" id="form-search" action="javascipr:void(0);">
                ...
                <li class="nav-item">
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu shadow shadow-sm" aria-labelledby="dropdownMenuButton" style="right: .5rem;left: unset;">
                            <a class="dropdown-item" href="{{url('/author/account')}}">Account</a>
                            <a class="dropdown-item" href="{{ url('/author/logout') }}">Logout</a>
                        </div>
                    </div>
                </li>
            </ul>
...
<div class="wrapper">
    <nav id="sidebar">
        <ul class="list-unstyled components pr-1 pt-5">
            <li id="i_dashboard"><a href="{{url('/author')}}" class="pl-sidebar"><i class="fas fa-home fa-lg mr-3"></i> Dashboard</a></li>
            <li id="i_notification"><a href="{{url('/author/notification')}}" class="pl-sidebar"><i class="far fa-bell fa-lg mr-3"></i>Notification <span class="badge badge-warning text-light right" id="ttl_notif" style="display: none">0</span></a></li>
            <li id="i_journal" class="d-none"><a href="{{url('/author/journal')}}" class="pl-sidebar"><i class="far fa-copy fa-lg mr-3"></i> Journals</a></li>
            <li id="i_my_journal"><a href="{{url('/author/my_journal')}}" class="pl-sidebar"><i class="far fa-file fa-lg mr-3"></i> My Journals</a></li>
            <li id="i_videos"><a href="{{url('/author/videos')}}" class="pl-sidebar"><i class="far fa-file-video fa-lg mr-3"></i> Videos</a></li>
            <li id="i_my_question"><a href="{{url('/author/my_question')}}" class="pl-sidebar"><i class="far fa-comment-dots fa-lg mr-3"></i> My Qestions</a></li>
            <li id="i_qa_forum"><a href="{{url('/author/qa_forum')}}" class="pl-sidebar"><i class="far fa-comments fa-lg mr-3"></i> QA Forums</a></li>
            <li id="i_revision"><a href="{{url('/author/revision')}}" class="pl-sidebar"><i class="far fa-comment-alt fa-lg mr-3"></i> Revisions</a></li>
        </ul>
    </nav>
    ...
    <div id="content">
        ...
        <footer class="footer mt-auto py-3 shadow-sm-footer p-3 bg-white">
            <div class="container">
                <span class="text-dark">Tegal International Conference</span>
            </div>
        </footer>
    </div>
</div>
...
<script type="text/javascript" src="{{ asset('/custom/custom_user.js?version=1')}}"></script>
<script type="text/javascript" src="{{ asset('/custom/custom_author.js?version=2')}}"></script>
<script type="text/javascript" src="{{asset('custom/validation.js?version=1')}}"></script>
<script type="text/javascript" src="{{asset('/plugins/autosize-master/dist/autosize.min.js')}}"></script>
<script type="text/javascript">
    function modal(page,str1,str2) {
        loader_show();
        $.ajax({
            type: "POST",headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
            data: {page:page,str1:str1,str2:str2}, url: "{{ url('/author/modal') }}",
    ...
    function getNotif() {
        $.ajax({
            url:'{{url('api/getNotifAuthor')}}',type: "GET",dataType: 'json',
    ...
    function setActionDelete(str) {
        if (str==="my_journal"||str==="my_question"||str==="notification_author"){
            reloadTable();
        }
    }
</script>
