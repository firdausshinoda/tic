    ...
    <script type="text/javascript">
        ...
        function getData() {
            $('#dt_table').DataTable({
                ...
                ajax: {
                    url: "{{ url('/api/dtTableMyQuestionAuthor') }}",
                    data: function (d) {
                        loader_show();
                    },
                ...
                columns: [
                    ...
                    { "render": function ( data, type, row )
                        {
                            return '<a href="{{url('/author/my_question/detail?abs=')}}'+row.no_abstrak+'&kd='+row.kode_qa+'">'+row.judul_jurnal+'</a>';
                        }
                    },
                    ...
                    { "render": function ( data, type, row )
                        {
                            return '<a href="{{url('/author/my_question/edit?abs=')}}'+row.no_abstrak+'" class="btn btn-sm btn-info m-1 btn-block">CHANGE</a>' +
                                '<button type="button" class="btn btn-sm btn-danger m-1 btn-block" onclick="ajaxDelete(\'my_question\',\''+row.kode_qa+'\')">DELETE</button>'
                        }
                    },
                ]
            });
