    ...
    <script type="text/javascript">
        ...
        function getData() {
            $('#dt_table').DataTable({
                ...
                ajax: {
                    url: "{{ url('/api/dtTableNotifParticipan') }}",
                    data: function (d) {
                        loader_show();
                    },
                    ...
                    { "render": function ( data, type, row )
                        {
                            var view = '';
                            if (row.stt_view===0){
                                view += '<button type="button" class="btn btn-sm btn-success m-1 btn-block" onclick="confirm_notif(\''+row.id_notif_participan+'\')"><i class="fas fa-check"></i></button>';
                            }
                            return view + '<button type="button" class="btn btn-sm btn-danger m-1 btn-block" onclick="ajaxDelete(\'notification_participan\',\''+row.id_notif_participan+'\')"><i class="far fa-trash-alt"></i></button>'
                        }
                    },
                ]
            });
            ...
            swalWithBootstrapButtons.fire({
                ...
                preConfirm: function () {
                    return new Promise(function (resolve) {
                        loader_show();
                        $.ajax({
                            type: "POST",data:{id:id}, url: "{{ url('/api/updateNotifParticipan') }}",
