            ...
            $.ajax({
                ...
                success: function(response)
                {
                    ...
                    if (response.status==="OK"){
                        ...
                        if (dt_jurnal.stt_full_paper==="ACCEPTED"){
                            $('#btn_revision').hide();
                        }
                    ...
                    { "render": function ( data, type, row )
                        {
                            if (row.DT_RowIndex === 1){
                                if (stt_paper !== "COPYEDITING" && row.file_revisi_author === null){
                                    return '<button type="button" class="btn btn-sm btn-danger m-1 btn-block" onclick="ajaxDelete(\'jurnal_revisi\',\''+row.id_jurnal_revisi+'\')">DELETE</button>';
                                } else {
                                    return '<button type="button" class="btn btn-sm btn-danger m-1 btn-block" disabled>DELETE</button>';
                                }
                            } else {
                                return '';
                            }
