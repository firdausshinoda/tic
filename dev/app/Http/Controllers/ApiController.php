...
class ApiController extends Controller
{
    ...
    public function journal(Request $request){
        ...
        $where['tb_jurnal.stt_jurnal'] = "COPYEDITING";
    ...
    public function detailJournal(Request $request){
        $select = array('link_video','no_abstrak','judul_jurnal','judul_jurnal');
    ...
    public function insertQAForum(Request $request){
        ...
        if ($stmt){
            if (Session::has('id_author')){
                if ($request->et_author != Session::get('id_author')){
                    $input2['id_author'] = Session::get('id_author');
