...
//             return "Gagal Semua";
//         }
        $stmt = TbJurnalModel::get();
        foreach ($stmt as $item) {
            echo $item->id_jurnal."<br/>";
            $where['id_jurnal'] = $item->id_jurnal;
            $update['abstrak_jurnal_normal'] = strip_tags($item->abstrak_jurnal);
            TbJurnalModel::where($where)->update($update);
        }
    }
